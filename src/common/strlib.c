...
int sv_parse_next(struct s_svstate* svstate)
{
	...
	{
		ShowError("sv_parse_next: delimitador '\\n' nao compativel com as opcoes SV_TERMINATE_LF ou SV_TERMINATE_CRLF.\n");
		return -1;// error
	}
	if( delim == '\r' && (opt&(SV_TERMINATE_CRLF|SV_TERMINATE_CR)) )
	{
		ShowError("sv_parse_next: delimitador '\\r' nao compativel com as opcoes SV_TERMINATE_CR ou SV_TERMINATE_CRLF.\n");
		return -1;// error
	}
		...
		case PARSING_C_ESCAPE:// skip escape sequence (validates it too)
			{
				...
				if( IS_END() )
				{
					ShowError("sv_parse_next: seqÅuencia de escape vazia\n");
					return -1;
				}
				if( str[i] == 'x' )
				{// hex escape
					...
					{
						ShowError("sv_parse_next: \\x sem digitos hexadecimais\n");
						return -1;
					}
					do{
						++i;// hex digit
				...
				{
					ShowError("sv_parse_next: seqÅuencia de escape desconhecida \\%c\n", str[i]);
					return -1;
				}
				state = PARSING_FIELD;
				break;
...
int sv_split(char* str, int len, int startoff, char delim, char** out_fields, int nfields, enum e_svopt opt) {
	...
	if( end[0] == '\0' ) {
		...
		*out_fields = end + 1;
		...
		ShowError("sv_split: linha de delimitador desconhecida 0x02%x.\n", (unsigned char)end[0]);
		return -1;// error
	}
	++out_fields;
	--nfields;
...
size_t sv_unescape_c(char* out_dest, const char* src, size_t len) {
	...
	for( i = 0, j = 0; i < len; ) {
		if( src[i] == '\\' ) {
			++i;// '\\'
			if( i >= len )
				ShowWarning("sv_unescape_c: seqÅuencia de escape vazia\n");
				...
				if( i >= len || !ISXDIGIT(src[i]) ) {
					ShowWarning("sv_unescape_c: \\x sem digitos hexadecimais\n");
					continue;
				}
				do {
					if( c > 0x0F && inrange ) {
						ShowWarning("sv_unescape_c: seqÅuencia de escape hexadecimal fora do limite\n");
						inrange = 0;
					}
				...
				if( strchr(SV_ESCAPE_C_SUPPORTED, src[i]) == NULL )
					ShowWarning("sv_unescape_c: seqÅuencia de escape desconhecida \\%c\n", src[i]);
				switch( src[i] ) {
					case 'a': out_dest[j++] = '\a'; break;
					case 'b': out_dest[j++] = '\b'; break;
...
bool sv_readdb(const char* directory, const char* filename, char delim, int mincols, int maxcols, int maxrows, bool (*parseproc)(char* fields[], int columns, int current)) {
	...
	if( (fp = fopen(path, "r")) == NULL ) {
		ShowError("sv_readdb: nao foi possivel ler %s\n", path);
		return false;
	}
	...
	while( fgets(line, sizeof(line), fp) ) {
		...
		if( columns < mincols ) {
			ShowError("sv_readdb: Columas insuficientes na linha %d de \"%s\" (encontrado(s) %d, precisa de pelo menos %d).\n", lines, path, columns, mincols);
			continue; // not enough columns
		}
		if( columns > maxcols ) {
			ShowError("sv_readdb: Muitas colunas em %d de \"%s\" (encontrado(s) %d, limite %d).\n", lines, path, columns, maxcols );
			continue; // too many columns
		}
		if( entries == maxrows ) {
			ShowError("sv_readdb: Atingido o numero maximo de entradas permitidas em (%d) ao analisar arquivo \"%s\".\n", maxrows, path);
			break;
		}

		// parse this row
		if( !parseproc(fields+1, columns, entries) ) {
			ShowError("sv_readdb: Nao foi possivel processar o conteudo da linha %d de \"%s\".\n", lines, path);
