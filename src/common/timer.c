...
int timer_add_func_list(TimerFunc func, char* name) {
	...
	if (name) {
		...
		{// check suspicious cases
			if( func == tfl->func )
				ShowWarning("timer_add_func_list: duplicando funcao %p(%s) como %s.\n",tfl->func,tfl->name,name);
			else if( strcmp(name,tfl->name) == 0 )
				ShowWarning("timer_add_func_list: funcao %p tem o mesmo nome %p(%s)\n",func,tfl->func,tfl->name);
		}
...
static void rdtsc_calibrate(void){
	...
	ShowStatus("Sincronizando o timer, aguarde por favor... ");
	...
	ShowMessage(" finalizado. (Frequencia: %u Mhz)\n", (uint32)(RDTSC_CLOCK/1000) );
}
...
int64 timer_settick(int tid, int64 tick)
{
	...
	if( i == BHEAP_LENGTH(timer_heap) ) {
		ShowError("timer_settick: timer nao existe %d (%p(%s))\n", tid, timer_data[tid].func, search_timer_func_list(timer_data[tid].func));
		return -1;
	}
