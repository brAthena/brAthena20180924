...
static void db_free_add(DBMap_impl* db, DBNode *node, DBNode **root)
{
	...
	if (db->free_lock == (unsigned int)~0) {
		ShowFatalError("db_free_add: free_lock overflow\n"
				"Banco de dados alocados na %s:%d\n",
...
static void db_free_remove(DBMap_impl* db, DBNode *node)
{
	...
	node->deleted = 0;
	if (i == db->free_count) {
		ShowWarning("db_free_remove: node nao foi encontrado - banco de dados alocados na %s:%d\n", db->alloc_file, db->alloc_line);
	} else {
		db->free_count--;
	}
	db->item_count++;
...
static void db_free_unlock(DBMap_impl* db)
{
	...
	if (db->free_lock == 0) {
		ShowWarning("db_free_unlock: free_lock e igual 0\n"
				"Banco de dados alocados na %s:%d\n",
				db->alloc_file, db->alloc_line);
	} else {
		db->free_lock--;
...
#if defined(DEBUG)
		if (db->cache->deleted) {
			ShowDebug("db_exists: Cache contem um node excluido. Por favor reporte isso!!!\n");
			return false;
		}
#endif
...
static DBData* db_obj_get(DBMap* self, DBKey key)
{
	...
	if (!(db->options&DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
		ShowError("db_get: Tentou recuperar um nao permitido NULL key para db alocado na %s:%d\n",db->alloc_file, db->alloc_line);
...
#if defined(DEBUG)
		if (db->cache->deleted) {
			ShowDebug("db_get: Cache contem um node excluido. Por favor reporte isso!!!\n");
			return NULL;
		}
#endif
...
static DBData* db_obj_vensure(DBMap* self, DBKey key, DBCreateData create, va_list args)
{
	...
	if (db == NULL) return NULL; // nullpo candidate
	if (create == NULL) {
		ShowError("db_ensure: Criar funcao e NULL para db alocado em %s:%d\n",db->alloc_file, db->alloc_line);
		return NULL; // nullpo candidate
	}
	if (!(db->options&DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
		ShowError("db_ensure: Tentou recuperar um nao permitido NULL key para db alocado na %s:%d\n",db->alloc_file, db->alloc_line);
		return NULL; // nullpo candidate
	}
	...
	if (node == NULL) {
		...
		if (db->item_count == UINT32_MAX) {
			ShowError("db_vensure: item_count overflow, abortando insercao.\n"
					"Banco de dados alocados na %s:%d",
					db->alloc_file, db->alloc_line);
				return NULL;
		}
...
static int db_obj_put(DBMap* self, DBKey key, DBData data, DBData *out_data)
{
	...
	if (db == NULL) return 0; // nullpo candidate
	if (db->global_lock) {
		ShowError("db_put: Banco de dados esta sendo destruido, abortando insercao.\n"
				"Banco de dados alocados na %s:%d\n",
				db->alloc_file, db->alloc_line);
		return 0; // nullpo candidate
	}
	if (!(db->options&DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
		ShowError("db_put: Tentou recuperar um nao permitido NULL key para db alocado na %s:%d\n",db->alloc_file, db->alloc_line);
		return 0; // nullpo candidate
	}
	...
	if (db->item_count == UINT32_MAX) {
		ShowError("db_put: item_count overflow, abortando insercao\n"
				"Banco de dados alocados na %s:%d",
				db->alloc_file, db->alloc_line);
		return 0;
	}
...
static int db_obj_remove(DBMap* self, DBKey key, DBData *out_data)
{
	...
	if (db == NULL) return 0; // nullpo candidate
	if (db->global_lock) {
		ShowError("db_remove: Banco de dados esta sendo destruido. Abortando eliminacao.\n"
				"Banco de dados alocados na %s:%d\n",
				db->alloc_file, db->alloc_line);
		return 0; // nullpo candidate
	}
	if (!(db->options&DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
		ShowError("db_remove: Tentou recuperar um nao permitido NULL key para db alocado na %s:%d\n",db->alloc_file, db->alloc_line);
		return 0; // nullpo candidate
	}
...
static int db_obj_vforeach(DBMap* self, DBApply func, va_list args)
{
	...
	if (db == NULL) return 0; // nullpo candidate
	if (func == NULL) {
		ShowError("db_foreach: Funcao passada e NULL para db alocada em %s:%d\n",db->alloc_file, db->alloc_line);
		return 0; // nullpo candidate
	}
...
static int db_obj_vdestroy(DBMap* self, DBApply func, va_list args)
{
	...
	if (db == NULL) return 0; // nullpo candidate
	if (db->global_lock) {
		ShowError("db_vdestroy: Banco de dados ja esta bloqueado para a destruicao. Anulando segunda destruicao de banco de dados.\n"
				"Banco de dados alocados na %s:%d\n",
				db->alloc_file, db->alloc_line);
		return 0;
	}
	if (db->free_lock)
		ShowWarning("db_vdestroy: Banco de dados ainda esta em uso, %u bloqueado(s). Continuando a destruicao do banco de dados.\n"
				"Banco de dados alocados na %s:%d\n",
...
DBReleaser db_custom_release(DBRelease which)
{
	...
	switch (which) {
		...
		default:
			ShowError("db_custom_release: Opcoes de leberacao desconhecidos %u\n", which);
			return NULL;
	}
}
...
void db_final(void)
{
	...
	ShowInfo(CL_WHITE"Nodes de bancos de dados"CL_RESET":\n"
			"alocado %u, liberado %u\n",
			stats.db_node_alloc, stats.db_node_free);
	ShowInfo(CL_WHITE"Tipos de bancos de dados"CL_RESET":\n"
