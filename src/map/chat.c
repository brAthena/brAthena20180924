...
******************************************************************************
* src/map/chat.c                                                             *
* Funções referentes às salas de chat dentro do jogo                         *
******************************************************************************
...
bool chat_createnpcchat(struct npc_data* nd, const char* title, int limit, bool pub, int trigger, const char* ev, int zeny, int minLvl, int maxLvl)
{
	...
	if( nd->chat_id ) {
		ShowError("chat_createnpcchat: npc '%s' ja tem uma chatroom, nao e possivel criar um novo!\n", nd->exname);
		return false;
	}

	if( zeny > MAX_ZENY || maxLvl > MAX_LEVEL ) {
		ShowError("chat_createnpcchat: npc '%s' tem um level ou uma quantidade de zeny acima do maximo permitido!\n", nd->exname);
		return false;
	}
...
/**
 * Retorna quantas chats existem no mapa informado. [CarlosHenrq]
 *
 * @param int16 m indice do mapa.
 *
 * @return int Quantidade de chats no mapa.
 */
...
/**
 * Retorna quantas chats existem na área informada. [CarlosHenrq]
 *
 * @param int16 m indice do mapa.
 * @param int16 x0
 * @param int16 y0
 ...
 * @param int16 x2
 *
 * @return int Quantidade de chats dentro da área.
 */
...
/**
 * Verifica se a celula que o jogador irá tentar criar uma chat já possui um jogador com chat em aberto.
 *
 * @param struct map_session_data* sd
 * 
 * @return boolean
 */
bool chat_cell_has_taken(struct map_session_data* sd)
{
	return (chat->count_cell(sd->bl.m, sd->bl.x, sd->bl.y) > 0);
}
