...
bool buyingstore_setup(struct map_session_data* sd, unsigned char slots)
{
	...
	if( !battle_config.buying_store || sd->state.vending || sd->state.buyingstore || sd->state.trading || slots == 0 )
	{
		return false;
	}
...
void buyingstore_create(struct map_session_data* sd, int zenylimit, unsigned char result, const char* storename, const uint8* itemlist, unsigned int count)
{
	...
	if( !battle_config.buying_store || pc_istrading(sd) || sd->buyingstore.slots == 0 || count > sd->buyingstore.slots || zenylimit <= 0 || zenylimit > sd->status.zeny || !storename[0] )
	...
	for (i = 0; i < count; i++) {
		...
		{// duplicate check. as the client does this too, only malicious intent should be caught here
			...
			if( listidx != i )
			{// duplicate
				ShowWarning("buyingstore_create: Item duplicado (nameid=%hu, quant=%hu, account_id=%d, char_id=%d).\n", nameid, amount, sd->status.account_id, sd->status.char_id);
				break;
			}
		}
...
void buyingstore_open(struct map_session_data* sd, int account_id)
{
	...
	nullpo_retv(sd);
	if( !battle_config.buying_store || pc_istrading(sd) )
	{// not allowed to sell
		return;
	}
...
void buyingstore_trade(struct map_session_data* sd, int account_id, unsigned int buyer_id, const uint8* itemlist, unsigned int count)
{
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		{// duplicate check. as the client does this too, only malicious intent should be caught here
			...
			if( k != i )
			{// duplicate
				ShowWarning("buyingstore_trade: Item duplicado (prevnameid=%hu, prevamount=%hu, nameid=%hu, quant=%hu, account_id=%d, char_id=%d).\n",
	...
	{// itemlist: <index>.W <name id>.W <amount>.W
		...
		// move item
		logs->buyingstore(pl_sd,sd,&sd->status.inventory[index],pl_sd->buyingstore.items[listidx].price,amount);
		pc->additem(pl_sd, &sd->status.inventory[index], amount);
		pc->delitem(sd, index, amount, 1, DELITEM_NORMAL);
		pl_sd->buyingstore.items[listidx].amount-= amount;

		// pay up
		pc->payzeny(pl_sd, zeny,"bStore", sd);
		pc->getzeny(sd, zeny, "bStore", pl_sd);
