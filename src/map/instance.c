...
******************************************************************************
* src/map/instance.c                                                         *
* Fun��es respons�veis pelo funcionamento das inst�ncias dentro do emulador  *
******************************************************************************
	...
	switch ( type ) {
		...
		case IOT_CHAR:
			if( ( sd = map->id2sd(owner_id) ) == NULL ) {
				ShowError("instance_create: personagem %d nao encontrado pra instancia '%s'.\n", owner_id, name);
				return -2;
			}
			iptr = sd->instance;
			icptr = &sd->instances;
		...
		case IOT_PARTY:
			if( ( p = party->search(owner_id) ) == NULL ) {
				ShowError("instance_create: grupo %d nao encontrado pra instancia '%s'.\n", owner_id, name);
				return -2;
			}
			iptr = p->instance;
			icptr = &p->instances;
		...
		case IOT_GUILD:
			if( ( g = guild->search(owner_id) ) == NULL ) {
				ShowError("instance_create: cla %d nao encontrado pra instancia '%s'.\n", owner_id, name);
				return -2;
			}
			iptr = g->instance;
			icptr = &g->instances;
			break;
		default:
			ShowError("instance_create: tipo desconhecido %d para owner_id %d nome %s.\n", type,owner_id,name);
			return -1;
	}
...
int instance_add_map(const char *name, int instance_id, bool usebasename, const char *map_name) {
	...
	if( !instance->valid(instance_id) ) {
		ShowError("instance_add_map: tentando atachar '%s' mapa para instancia nao-existente %d.\n", name, instance_id);
		return -1;
	}
	...
	if( map->list[m].instance_id >= 0 ) {
		...
		ShowError("instance_add_map: tentando criar instancia em mapa ja instanciado %s.\n", name);
		return -4;
	}
	...
	if( !map->list[im].index ) {
		...
		ShowError("instance_add_map: sem indices disponiveis.\n");
...
void instance_del_map(int16 m) {
	...
	if( m <= 0 || map->list[m].instance_id == -1 ) {
		ShowError("instance_del_map: tentou remover mapa de instancia nao-existente (%d)\n", m);
		return;
	}
	...
	if( i == instance->list[map->list[m].instance_id].num_map )
		ShowError("map_instance_del: nao conseguiu remover %s da lista de instancias (%s): %d\n", map->list[m].name, instance->list[map->list[m].instance_id].name, m);
