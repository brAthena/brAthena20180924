	...
	if (groups != NULL) {
		...
		for (i = 0; i < group_count; ++i) {
			...
			if (!libconfig->setting_lookup_int(group, "id", &id)) {
				ShowConfigWarning(group, "pc_groups:read_config: membro da lista \"groups\" tem um id #%d indefinido, removendo...", i);
			...
			if (!libconfig->setting_lookup_string(group, "name", &groupname)) {
				...
				if ((name = config_setting_add(group, "name", CONFIG_TYPE_STRING)) == NULL ||
					...
					ShowError("pc_groups:read_config: Falha em definir o nome do grupo que faltava, id=%d, pulando... (%s:%d)\n",
		...
		for (group_settings = dbi_first(iter); dbi_exists(iter); group_settings = dbi_next(iter)) {
			...
			for (i = 0; i < count; ++i) {
				...
				if (!atcommand->exists(name)) {
					ShowConfigWarning(command, "pc_groups:read_config: Nome do comando inexistente '%s', removendo...", name);
		...
		while (i < group_count) {
			...
			for (group_settings = dbi_first(iter); dbi_exists(iter); group_settings = dbi_next(iter)) {
				...
				for (j = 0; j < inherit_count; ++j) {
					...
					if (groupname == NULL) {
						ShowConfigWarning(inherit, "pc_groups:read_config: Matriz \"inherit\" #%d nao e um nome, removendo...", j);
						libconfig->setting_remove_elem(inherit,j);
						continue;
					}
					if ((inherited_group = name2group(groupname)) == NULL) {
						ShowConfigWarning(inherit, "pc_groups:read_config: Nome do grupo inexistente \"%s\", removendo...", groupname);
						libconfig->setting_remove_elem(inherit,j);
						continue;
					}
			...
			if (++loop > group_count) {
				ShowWarning("pc_groups:read_config: Nao foi possivel processar regras de heranca, verifique a sua config '%s'...\n",
				            config_filename);
				break;
			}
		} // while(i < group_count)
...
unsigned int pc_groups_add_permission(const char *name) {
	...
	for(i = 0; i < pcg->permission_count; i++) {
		if( strcmpi(name,pcg->permissions[i].name) == 0 ) {
			ShowError("pc_groups_add_permission(%s): falhou! Permissao duplicada!\n",name);
			return 0;
		}
	}
	...
	if( key >= UINT_MAX ) {
		ShowError("pc_groups_add_permission(%s): falhou! Sem espaco suficiente, muitas permissoes!\n",name);
		return 0;
	}
