...
******************************************************************************
* src/map/mob.c                                                              *
* Funções responsáveis pelo funcionamento geral de monstros                  *
******************************************************************************
...
int mob_parse_dataset(struct spawn_data *data)
{
	...
	if(!data->name){
		if(mob->db(data->class_) == NULL)
			ShowError("mob_parse_dataset - Monstro [ID = %d] nÃ£o existente na base de dados, ignorando...",data->class_);
		else
...
struct mob_data *mob_once_spawn_sub(struct block_list *bl, int16 m, int16 x, int16 y, const char *mobname, int class_, const char *event, unsigned int size, unsigned int ai)
{
	...
	if (mobname)
		...
		if(mob->db(class_) == NULL){
			ShowError("mob_once_spawn_sub : Monstro [id = %d] nÃ£o existente na base de dados...\n",class_);
			return NULL;
		}
		else{
...
void mob_name_constants(void) {
	...
	script->parser_current_file = "Banco de dados de Monstros (Provavelmente um nome de Sprite invÃ¡lido ou em conflitantes)";
		...
		while(i < MAX_RANDOMMONSTER && (SQL_SUCCESS == SQL->NextRow(map->brAmysql_handle))) {
			...
			if(strcmp(row[0],last_group)!=0){  /* New List Found!! Rework indexes */
				...
				if(n!=1 && summon[i].iteminfo==NULL){
					ShowError("%s - item : %s nÃ£o existente na base de dados\n",mobfile[n],row[0]);
					return 2;
				}
