...
int mob_parse_dataset(struct spawn_data *data)
{
	...
	if(!data->name){
		if(mob->db(data->class_) == NULL)
			ShowError("mob_parse_dataset - Monstro [ID = %d] n√£o existente na base de dados, ignorando...",data->class_);
		else
...
bool mob_ksprotected(struct block_list *src, struct block_list *target) {
	...
	do {
		...
		{
			sprintf(output, "[AtenÁ„o!! %s est· tentando fazer KS em vocÍ!]", sd->status.name);
...
struct mob_data *mob_once_spawn_sub(struct block_list *bl, int16 m, int16 x, int16 y, const char *mobname, int class_, const char *event, unsigned int size, unsigned int ai)
{
	...
	if (mobname)
		...
		if(mob->db(class_) == NULL){
			ShowError("mob_once_spawn_sub : Monstro [id = %d] n√£o existente na base de dados...\n",class_);
			return NULL;
		}
		else{
...
int mob_spawn_guardian_sub(int tid, int64 tick, int id, intptr_t data) {
	...
	if( bl->type != BL_MOB ) {
		ShowError("mob_spawn_guardian_sub: Erro no bloco!\n");
		return 0;
	}
...
int mob_spawn_guardian(const char* mapname, short x, short y, const char* mobname, int class_, const char* event, int guardian, bool has_index)
{
	...
	if( has_index && gc->guardian[guardian].id ) {
		...
		if (md2 && md2->bl.type == BL_MOB
			...
			ShowError("mob_spawn_guardian: Tentativa de invocar o guardiao %d porem ja existe neste mapa (mapa %s)\n", guardian, map->list[m].name);
			return 0;
		}
	}
...
int mob_ai_sub_hard_activesearch(struct block_list *bl,va_list ap)
{
	...
	switch (bl->type) {
		...
		default:
			if (battle_config.hom_setting&4 &&
...
void mob_name_constants(void) {
	...
	script->parser_current_file = "Banco de dados de Monstros (Provavelmente um nome de Sprite inv√°lido ou em conflitantes)";
		...
		while(i < MAX_RANDOMMONSTER && (SQL_SUCCESS == SQL->NextRow(map->brAmysql_handle))) {
			...
			if(strcmp(row[0],last_group)!=0){  /* New List Found!! Rework indexes */
				...
				if(n!=1 && summon[i].iteminfo==NULL){
					ShowError("%s - item : %s n√£o existente na base de dados\n",mobfile[n],row[0]);
					return 2;
				}
