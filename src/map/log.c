...
void log_branch(struct map_session_data *sd) {
	...
	if (logs->config.branch) {
		if (SQL_ERROR == SQL->Query(logs->mysql_handle, LOG_QUERY " INTO `%s` (`branch_date`, `account_id`, `char_id`, `char_name`, `map`) VALUES (NOW(), '%d', '%d', '%s', '%s')", logs->config.table_name[1], sd->status.account_id, sd->status.char_id, sd->status.name, mapindex_id2name(sd->mapindex))) {
...
void log_cash_buy_sql(struct map_session_data *sd, char *type, char *npc_name, struct item *itm, int amount, int price) {
	...
	if (logs->config.buycash) {
		...
		if (SQL_ERROR == SQL->Query(logs->mysql_handle, LOG_QUERY " INTO `%s` (`Date`, `Mapname`, `AccountID`, `AccountName`,`CharacterID`,`CharacterIPaddr`,`Type`,`PosX`,`PosY`,`NpcName`,\
