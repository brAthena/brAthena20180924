...
* Funções referentes aos Homunculus de alquimistas e evoluções               *
******************************************************************************
...
bool homunculus_levelup(struct homun_data *hd) {
	...
	if ( battle_config.homunculus_show_growth ) {
		char output[256] ;
		sprintf(output,
			"Crescimento: hp:%d sp:%d str(%.2f) agi(%.2f) vit(%.2f) int(%.2f) dex(%.2f) luk(%.2f) ",
...
bool homunculus_evolve(struct homun_data *hd) {
	...
	if (!homun->change_class(hd, hd->homunculusDB->evo_class)) {
		ShowError("homunculus_evolve: Nao foi possivel evoluir homun de %d para %d", hd->homunculus.class_, hd->homunculusDB->evo_class);
		return false;
	}
...
bool homunculus_mutate(struct homun_data *hd, int homun_id) {
	...
	if( !homun->change_class(hd, homun_id) ) {
		ShowError("homunculus_mutate: Nao foi possivel evoluir homun de %d para %d", hd->homunculus.class_, homun_id);
		return false;
	}
...
unsigned char homunculus_menu(struct map_session_data *sd,unsigned char menu_num) {
	...
	switch(menu_num) {
		...
		case 2:
			...
			break;
		default:
			ShowError("homunculus_menu : escolha desconhecida : %d\n", menu_num) ;
			break;
	}
	return 0;
}
...
bool homunculus_create(struct map_session_data *sd, struct s_homunculus *hom) {
	...
	if(i < 0) {
		ShowError("homunculus_create: classe desconhecida [%d] para '%s', deletando requisicao.\n", hom->class_, hom->name);
		sd->status.hom_id = 0;
...
bool homunculus_read_skill_db_sub(char* split[], int columns, int current) {
	...
	if (j == MAX_SKILL_TREE) {
		ShowWarning("Nao foi possivel carregar a habilidade %d na arvore %d'. Maximo de habilidades atingido.\n", k, classid);
		return false;
	}
