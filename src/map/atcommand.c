...
ACMD(mobinfo)
{
	...
	for (k = 0; k < count; k++) {
		...
		// stats
		if (monster->mexp)
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1240), monster->name, monster->jname, monster->sprite, monster->vd.class_); // MVP Monster: '%s'/'%s'/'%s' (%d)
		else
			safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1241), monster->name, monster->jname, monster->sprite, monster->vd.class_); // Monster: '%s'/'%s'/'%s' (%d)
		clif->message(fd, atcmd_output);
...
#else
		safesnprintf(atcmd_output, sizeof(atcmd_output), msg_txt(1244), //  ATK:%d~%d  Range:%d~%d~%d  Size:%s  Race: %s  Element: %s (Lv:%d)
...
ACMD(reloadcashshop) {
	...
	intif->broadcast(msg_txt(3000), strlen(msg_txt(3000)) + 1, 0);
