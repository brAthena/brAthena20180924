...
int ElementTable[4][ELE_MAX][ELE_MAX] =
{
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		...
		{	100,  25, 100, 150,  90, 100,  75, 100, 100, 100	},		// Ãgua LV.1
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		...
		{	100,   0, 100, 175,  80, 100,  50,  75, 100, 100	},		// Ãgua LV.2
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		...
		{	100, -25, 100, 200,  70, 100,  25,  50, 100, 125	},		// Ãgua LV.3
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		...
		{	100, -50, 100, 200,  60,  75,   0,  25, 100, 150	},		// Ãgua LV.4
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		...
		{	100,  25, 100, 150,  50, 100,  75, 100, 100, 100	},		// Ãgua LV.1
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		...
		{	100,   0, 100, 175,  25, 100,  50,  75, 100, 100	},		// Ãgua LV.2
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		{	100, 100, 100, 100, 100, 100, 100, 100,   0, 100	},		// Neutro LV.3
		{	100, -25, 100, 200,   0, 100,  25,  50, 100, 125	},		// Ãgua LV.3
	...
	{//		Neut Watr Erth Fire Wind Pois Holy Shdw Gho  Und
		{	100, 100, 100, 100, 100, 100, 100, 100,   0, 100	},		// Neutro LV.4
		{	100, -50, 100, 200,   0,  75,   0,  25, 100, 150	},		// Ãgua LV.4
...
int pc_bonus2(struct map_session_data *sd,int type,int type2,int val)
{
	...
	switch(type){
		...
		case SP_EXP_ADDRACE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_EXP_ADDRACE: Raca Invalida (%d)\n",type2);
				break;
			}
			if(sd->state.lr_flag != 2) {
				if ( type2 >= RC_MAX ){
		...
		case SP_SP_GAIN_RACE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_SP_GAIN_RACE: Raca Invalida (%d)\n",type2);
				break;
			}
			if(sd->state.lr_flag != 2) {
				if ( type2 >= RC_MAX ){
		...
		case SP_HP_DRAIN_VALUE_RACE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_HP_DRAIN_VALUE_RACE: Raca Invalida (%d)\n",type2);
				break;
			}
			if ( type2 >= RC_MAX ){
				for ( i = RC_FORMLESS; i < RC_BOSS; i++ ) {
		...
		case SP_SP_DRAIN_VALUE_RACE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_SP_DRAIN_VALUE_RACE: Raca Invalida (%d)\n",type2);
				break;
			}
			if ( type2 >= RC_MAX ){
				for ( i = RC_FORMLESS; i < RC_BOSS; i++ ) {
		...
		case SP_SP_GAIN_RACE_ATTACK:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_SP_GAIN_RACE_ATTACK: Raca Invalida (%d)\n",type2);
				break;
			}
			if(sd->state.lr_flag != 2) {
				if ( type2 >= RC_MAX ) {
		...
		case SP_HP_GAIN_RACE_ATTACK:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_HP_GAIN_RACE_ATTACK: Raca Invalida (%d)\n",type2);
				break;
			}
			if(sd->state.lr_flag != 2) {
				if ( type2 >= RC_MAX ) {
		...
		case SP_RACE_TOLERANCE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus2: SP_RACE_TOLERANCE: Raca Invalida (%d)\n",type2);
				break;
			}
			if(sd->state.lr_flag != 2) {
				if ( type2 >= RC_MAX ) {
...
int pc_bonus3(struct map_session_data *sd,int type,int type2,int type3,int val)
{
	...
	switch(type){
		...
		case SP_HP_DRAIN_RATE_RACE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus3: SP_HP_DRAIN_RATE_RACE: Raca Invalida (%d)\n",type2);
				break;
			}
			if ( type2 >= RC_MAX ) {
				for ( i = RC_FORMLESS; i < RC_BOSS; i++ ) {
		...
		case SP_SP_DRAIN_RATE_RACE:
			if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
				ShowWarning("pc_bonus3: SP_SP_DRAIN_RATE_RACE: Raca Invalida (%d)\n",type2);
				break;
			}
			if ( type2 >= RC_MAX ) {
				for ( i = RC_FORMLESS; i < RC_BOSS; i++ ) {
...
int pc_bonus4(struct map_session_data *sd,int type,int type2,int type3,int type4,int val) {
	...
	case SP_SET_DEF_RACE: //bonus4 bSetDefRace,n,x,r,y;
		if (type2 == RC_MAX || (type2 > RC_NONDEMIPLAYER && type2 != RC_ALL) || type2 < RC_FORMLESS ){
			ShowWarning("pc_bonus4: SP_SET_DEF_RACE: Raca Invalida (%d)\n",type2);
			break;
		}
		if(sd->state.lr_flag == 2)
			break;
...
unsigned int pc_baseexp_sub(int class_, unsigned int base_level)
{
	...
	{
		struct exp_table* e_tbl = (pc->exp_db + i);
...
/*==========================================
 * Realiza a leitura das tabelas de experiencia. [Carlos/brAthena]
...
int pc_read_exp_fromsql(void)
{
	...
	pc->exp_db = (struct exp_table*) malloc(sizeof(struct exp_table) * EXP_TABLE_READ);

	// Faz a leitura das 11 tabelas.
	for (i = 0, database_id = 34; i < EXP_TABLE_READ; database_id = 34 + (++i))
	{
		struct exp_table* exp_tmp = (pc->exp_db + i);
