...
******************************************************************************
* src/map/path.c                                                             *
* Funções responsáveis pelas verificações de caminhos no mapa                *
******************************************************************************
...
int path_blownpos(struct block_list *bl, int16 m,int16 x0,int16 y0,int16 dx,int16 dy,int count)
{
	...
	if( count>25 ){ //Cap to prevent too much processing...?
		ShowWarning("path_blownpos: contagem muito extensa %d !\n",count);
		count=25;
	}
	if( dx > 1 || dx < -1 || dy > 1 || dy < -1 ){
		ShowError("path_blownpos: ilegal dx=%d ou dy=%d !\n",dx,dy);
		dx=(dx>0)?1:((dx<0)?-1:0);
		dy=(dy>0)?1:((dy<0)?-1:0);
	}
