...
#ifdef NOISY
	ShowInfo("Salvar requisicao de grupo ("CL_BOLD"%d"CL_RESET" - %s).\n", party_id, p->name);
#endif
...
#ifdef NOISY
	ShowInfo("Carregando requisicao de grupo ("CL_BOLD"%d"CL_RESET")\n", party_id);
#endif
	if( party_id <= 0 )
		return NULL;
...
int inter_party_CharOnline(int char_id, int party_id)
{
	...
	p = inter_party->fromsql(party_id);
	if(!p) {
		ShowError("Personagem %d grupo %d nao encontrado!\n", char_id, party_id);
		return 0;
	}
